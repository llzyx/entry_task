...
import pytest
from variables import internal_destination,place_of_departure,country_index,letter_index,device_caps,module,type,route,aircraft_cabin
from appium.webdriver.common.appiumby import AppiumBy

#单程、出发地、目的地为国内、头等舱+无儿童/婴儿
def test_10():
    ...
    #断言是否跳转到结果页面
    ele=bp.findElement(['content_desc','两舱列表页头部出发城市'])
    assert ele.text==place_of_departure[1],'航班的出发城市错误'
    ele1 = bp.findElement(['content_desc','两舱列表页头部到达城市'])
    assert ele1.text == internal_destination[2], '航班的到达城市错误'
    if bp.findElement(['content_desc','第1条航班']):
        assert bp.findElement(['text',aircraft_cabin[1]]),'查询后舱位错误'
